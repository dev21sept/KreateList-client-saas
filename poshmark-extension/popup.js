...
function setConnectedState(site, username, csrfToken) {
  ...
  if (site === 'poshmark') {
    ...
    btnFetchCloset.innerHTML = `
      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="btn-icon"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
      Fetch Sizes (1773)
    `;
    // For Vinted size check, you don't necessarily have to be logged in (sometimes works as guest too)
    btnFetchCloset.disabled = false;
...
async function checkActiveConnection() {
  try {
    ...
    if (tabs.length === 0) {
      ...
      log('No matching Poshmark/Vinted tab found. Open a site tab to begin.', 'warning');
      return;
    }
