    ...
    if (imageInput.startsWith('data:')) {
        ...
        try {
            ...
            if (fs.existsSync(filepath)) {
                ...
                console.log(`[AI Controller] Successfully read local file from uploads: ${filepath}`);
            } else {
                console.error(`[AI Controller] Local file not found: ${filepath}`);
            }
        } catch (err) {
            console.error(`[AI Controller] Error reading local file:`, err.message);
        }
    }
    ...
    if (!buffer) {
        if (imageInput.startsWith('http://localhost') || imageInput.startsWith('http://127.0.0.1')) {
            try {
                ...
                console.log(`[AI Controller] Fetched localhost image via HTTP: ${imageInput}`);
            } catch (err) {
                console.error(`[AI Controller] Failed to fetch localhost image via HTTP:`, err.message);
            }
        }
    }
