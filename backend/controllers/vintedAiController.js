    ...
    if (imageInput.startsWith('data:')) {
        ...
        try {
            ...
            if (fs.existsSync(filepath)) {
                ...
                console.log(`[Vinted AI] Successfully read local file from uploads: ${filepath}`);
            } else {
                console.error(`[Vinted AI] Local file not found: ${filepath}`);
            }
        } catch (err) {
            console.error(`[Vinted AI] Error reading local file:`, err.message);
        }
    }
    ...
    if (!buffer) {
        if (imageInput.startsWith('http://localhost') || imageInput.startsWith('http://127.0.0.1')) {
            try {
                ...
                console.log(`[Vinted AI] Fetched localhost image via HTTP: ${imageInput}`);
            } catch (err) {
                console.error(`[Vinted AI] Failed to fetch localhost image via HTTP:`, err.message);
            }
        }
    }
