...
async function publishSingleEbayListing(listing, token) {
  try {
    ...
    let offerId = createOfferRes.offerId;
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if ((errId === 25604 || errMsg.includes('product not found')) && attempt < 3) {
          console.warn(`[BULK EBAY PUBLISH] eBay replication lag detected (Product not found). Retrying in 3 seconds... (Attempt ${attempt}/3)`);
          await new Promise(resolve => setTimeout(resolve, 3000));
        } else if ((errId === 25008 || errMsg.includes('invalid category') || errMsg.includes('category is not valid')) && attempt < 3) {
          console.warn(`[BULK EBAY PUBLISH] Category ID rejected during publishOffer for "${listing.title}". Auto-resolving valid leaf category...`);
          try {
            const suggestions = await ebayService.getCategorySuggestions(token, listing.title || 'clothing');
            if (suggestions && suggestions.length > 0 && suggestions[0].category?.categoryId) {
              const validCatId = String(suggestions[0].category.categoryId);
              console.log(`[BULK EBAY PUBLISH] Recreating offer with valid leaf category: ${validCatId} (${suggestions[0].category.categoryName})`);
            ...
            console.error('[BULK EBAY PUBLISH] Category auto-resolve retry failed:', suggestErr.message);
          }
          throw pubErr;
        } else {
          throw pubErr;
